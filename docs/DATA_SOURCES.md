...
- **L2-глубина по WS для Density** (`mexc_monitor/ws_futures_depth_book.py`): полный стакан топ-20 уровней MEXC USDT-перпов приходит по каналу `sub.depth.full` (самодостаточные снапшоты, не дельты — в отличие от L1-фида `ws_futures_orderbook.py`). Объём контрактов конвертируется в истинный USDT через `contract/detail` (`contractSize`, кешируется 1 ч; эндпоинт доступен даже при геоблоке market-data REST). Density-эндпоинты (`/api/density/walls|stats|heatmap|overview`) и `/api/depth` предпочитают WS-книгу и падают на REST → Binance. Подписка растёт по требованию: `/api/density/overview` помечает часто просматриваемые символы (`touch_watchlist` + `reconcile`, лимит 30 на соединение). Состояние — в `/api/health` под `orderbook_ws.mexc_futures_depth`. Поле `source` в ответах показывает фактический источник (`ws`/`rest`/`binance_fallback`).
- **L2-глубина OKX и Bybit по WS** (`mexc_monitor/ws_l2_depth.py`): полная книга по WS со **снапшот + дельта** reassembly (в отличие от самодостаточного MEXC-канала). OKX `books` (400 уровней, `action=snapshot`→`update`), размер в контрактах → USDT через `ctVal` из `/api/v5/public/instruments` (кешируется); Bybit `orderbook.50` (50 уровней, `type=snapshot`→`delta`), размер уже в базовой монете (множитель 1.0). Дельта: `size == 0` — удалить уровень, иначе вставка/замена по цене. `density_overview` для okx/bybit предпочитает эту книгу (watchlist + reconcile, лимит 40 на соединение); здоровье — в `/api/health` под `orderbook_ws.okx_l2_depth` / `bybit_l2_depth`. **OKX работает полностью** (WS+REST доступны). **Bybit:** WS-хост `stream.bybit.com` жив, но REST `api.bybit.com`/`api.bytick.com` геоблокируется CloudFront 403 — список символов и объёмы для overview берутся из REST, поэтому Bybit-overview из песочницы деградирует с понятным сообщением; сама WS-книга рабочая и оживает вместе с overview при подключении прокси (см. §6 / `docs/PROXY_XRAY.md`).
...
- **Управление:** страница **Сеть / Прокси** (`/network`) — общий прокси + таблица переопределений по биржам с индивидуальной проверкой связи. API: `GET/PATCH /api/network/config`, `GET /api/network/test?exchange=<name>` — все admin only (`X-Admin-Token`), т.к. ответы содержат прокси-URL с креденшелами. В конфиге: `mexc.http_proxy_url` (общий) и `mexc.http_proxy_per_exchange` (`{биржа: url|"direct"}`).
- **Env-переменные** (удобно на Vercel/Docker — «вписал прокси» без правки JSON): `MEXC_HTTP_PROXY` — общий прокси; `MEXC_HTTP_PROXY_<БИРЖА>` — per-exchange (напр. `MEXC_HTTP_PROXY_BINANCE=socks5h://host:1080`, `MEXC_HTTP_PROXY_OKX=direct`). Env имеет приоритет над JSON для тех же бирж; на старте реестр сидится из объединённого конфига. Значения можно менять в рантайме через страницу/PATCH без рестарта.
- **Проверено end-to-end:** трафик биржи реально туннелируется через назначенный прокси — и probe (`/api/network/test`), и реальный путь данных (`/api/snapshot`) дают `CONNECT <exchange-host>:443` в логе прокси; поля `proxy`/`source` в ответах отражают фактический маршрут.
...
- **VLESS-прокси:** напрямую вписать нельзя (это транспорт Xray/V2Ray, не HTTP/SOCKS). Поднимите локальный мост `scripts/xray_bridge.py` (VLESS → `socks5h://127.0.0.1:10808`) и пропишите его биржам как обычный SOCKS-прокси. Подробный гайд: **`docs/PROXY_XRAY.md`**.
