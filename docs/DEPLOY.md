...
| `ADMIN_TOKEN_LOCAL_BOOTSTRAP` | нет | `0` (значение по умолчанию в compose) выключает `GET /api/admin-token`. В проде токен вводится вручную: **Trading Admin → X-Admin-Token**. |
...
> **Про `/api/admin-token`.** Эндпоинт существует только для локального dev-запуска
> (Vite на 5173 + API на 8006), чтобы UI не спрашивал токен на каждой перезагрузке.
> Проверки «клиент — 127.0.0.1» недостаточно: при reverse-proxy на том же хосте
> внешний запрос приходит с петлевого адреса. Поэтому эндпоинт дополнительно
> отказывает при наличии `X-Forwarded-*`/`X-Real-IP`/`Forwarded` и полностью
> выключается через `ADMIN_TOKEN_LOCAL_BOOTSTRAP=0`.
...
```bash
# /api/network/* — admin only: ответ раскрывает прокси-URL (обычно с креденшелами)
curl -H "X-Admin-Token: $ADMIN_TOKEN" \
  "http://localhost:8006/api/network/test?exchange=binance"
```
