...
export function AsterDexPage() {
  ...
  // ─── Effects: load data on mount and tab change ────────────────────────

  useEffect(() => {
    if (tab === "tickers") fetchTickers();
    if (tab === "funding") fetchFunding();
    if (tab === "cross") fetchCrossSpread();
  }, [tab, fetchTickers, fetchFunding, fetchCrossSpread]);
  ...
  }, [autoRefresh, tab, fetchTickers, fetchFunding, fetchCrossSpread]);
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  {loading && filteredTickers.length === 0 && (
                    <SkeletonTableRows rows={10} colSpan={7} />
                  )}
                ...
                <div className="space-y-4">
                  {crossResults.map((cr) => (
                    ...
                    >
                      ...
                      {cr.cross_spread && (
                        <div className="grid grid-cols-1 gap-3 pt-2 border-t border-line/50 md:grid-cols-2">
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  ...
                  {funding
                    ...
                    .slice(0, 200)
                    .map((f) => {
                      const annualized = f.last_funding_rate * 3 * 365 * 100;
